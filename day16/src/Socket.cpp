...
Socket::Socket(int fd) : fd_(fd) { ErrorIf(fd == -1, "socket create error"); }
...
int Socket::Accept(InetAddress* addr) {
    ...
    if(IsNonBlocking()) {
        while(true) {
            clnt_socketfd = accept(fd_, (sockaddr*)& tmp_addr, &addr_len);
        ...
        clnt_socketfd = accept(fd_, (sockaddr*)& tmp_addr, &addr_len);
        ErrorIf(clnt_socketfd == -1 , "socket accept error");
    }
    addr->SetAddr(tmp_addr);      //not check addr empty
